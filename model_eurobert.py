...
class EuroBertForToxicReasoning(EuroBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.cls = torch.nn.ModuleDict({
            'toxicity':             nn.Linear(config.hidden_size, 1),
            'counternarrative':     nn.Linear(config.hidden_size, 1),
            'justInappropriate':    nn.Linear(config.hidden_size, 1),
            'hasImplication':       nn.Linear(config.hidden_size, 1),
            'subject':              nn.Linear(config.hidden_size, 5),
            'subjectGroupType':     nn.Linear(config.hidden_size, 15),
            'subjectTokens':        nn.Linear(config.hidden_size, 1),
            'hasOther':             nn.Linear(config.hidden_size, 1),
            'other':                nn.Linear(config.hidden_size, 5),
            'otherTokens':          nn.Linear(config.hidden_size, 1),
            'implTopic':            nn.Linear(config.hidden_size, 7),
            'implTopicTokens':      nn.Linear(config.hidden_size, 1),
            'implPolarity':         nn.Linear(config.hidden_size, 3),
            'implTemporality':      nn.Linear(config.hidden_size, 3),
            'implStereotype':       nn.Linear(config.hidden_size, 1),
            'implSarcasm':          nn.Linear(config.hidden_size, 1),
            'authorBelief':         nn.Linear(config.hidden_size, 1),
            'authorPrefer':         nn.Linear(config.hidden_size, 1),
            'authorAccount':        nn.Linear(config.hidden_size, 1),
            'typicalBelief':        nn.Linear(config.hidden_size, 1),
            'typicalPrefer':        nn.Linear(config.hidden_size, 1),
            'expertBelief':         nn.Linear(config.hidden_size, 1),
        })
    ...
    def further_init(self, comment_token_id):
        ...
        if comment_token_id >= self.model.embed_tokens.num_embeddings:
            ...
            with torch.no_grad():
                self.model.embed_tokens.weight[self.comment_token_id] = s_embedding
